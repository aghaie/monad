...
def run(domain, unit_ref, worker_name="statistical", run_root=None):
    ...
    try:
        ...
        # ثبتِ فرضیه‌های رد‌شده
        for v in vpay["verifications"]:
            if v["decision"] == "REJECTED":
                reason = next(
                    (str(t) for t in v.get("tests", {}).values()
                     if isinstance(t, dict) and not t.get("passed", True)),
                    "failed verification")
                mem.record_rejected(run_id, v["hypothesis_id"], reason)
        ...
        mem.record_discovery(run_id, accepted_ids)
